...
/* Получение массива объявлений из cookie */
function getAdListFromCookie($cookie) {
    $adList = array();
    if (isset($cookie['adList'])) {
        $adList = unserialize($cookie['adList']);        
    }
    else {
        setcookie('adList', '', time()+3600*24*7);        
    }
    return $adList;
}
